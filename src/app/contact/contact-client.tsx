...
export default function ContactClient({ socialLinks }: { socialLinks: any[] }) {
  ...
  const getDescription = (platform: string): string => {
    const descriptions: Record<string, { ru: string; en: string }> = {
      telegram: { ru: 'Самый быстрый способ связаться', en: 'Fastest way to reach me' },
      email: { ru: 'Для официальных запросов', en: 'For formal inquiries' },
      github: { ru: 'Мой код', en: 'Check my code' },
      linkedin: { ru: 'Профессиональная сеть', en: 'Professional network' },
    }
    const desc = descriptions[platform]
    return desc ? (lang === 'ru' ? desc.ru : desc.en) : platform
  }

  const getName = (link: any): string => {
    const label = lang === 'ru' ? (link.labelRu || link.labelEn) : (link.labelEn || link.labelRu)
    return label || link.platform.charAt(0).toUpperCase() + link.platform.slice(1)
  }
  ...
  const contactMethods = socialLinks.map(link => ({
    ...
    href: link.url,
    badge: link.platform === 'telegram' ? (lang === 'ru' ? 'Preferred' : 'Preferred') : undefined,
  }))
    ...
    <div className="min-h-screen py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 p-6 bg-background-secondary border border-border-subtle rounded-lg">
          <h2 className="text-lg font-semibold mb-2">{t("contact.looking")}</h2>
